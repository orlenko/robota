import os
import subprocess

from rich.progress import Progress, SpinnerColumn, TimeElapsedColumn
from rich.prompt import Prompt
from slugify import slugify

from .jira import get_issue
from . import appstate
from .env import CHECKOUT_DIR, GITHUB_ORG, GITHUB_REPOS


def get_repos(workdir_path):
    repos = []
    while 1:
        repo = Prompt.ask(
            f"Which repo? ({' / '.join(GITHUB_REPOS)} or a different one)"
        )
        repos.append(repo)
        has_more = Prompt.ask("One more?", choices=["y", "n"], default="n")
        if has_more == "n":
            break

    jira_story_id = os.path.basename(workdir_path)

    with Progress(
        SpinnerColumn(),
        TimeElapsedColumn(),
        transient=True,
    ) as progress:
        task = progress.add_task(
            f"Getting the Jira story detals for {jira_story_id}", total=1
        )
        jira_story = get_issue(jira_story_id)
        progress.update(task, advance=1)

    branchname = f"vlad-{jira_story_id}-{slugify(jira_story.fields.summary)}"

    with Progress(
        SpinnerColumn(),
        TimeElapsedColumn(),
        transient=True,
    ) as progress:
        for repo in repos:
            task = progress.add_task(f"Checking out {GITHUB_ORG}/{repo}", total=1)
            result = subprocess.run(
                f""" \\
                cd {workdir_path} && \\
                git clone git@github.com:{GITHUB_ORG}/{repo}.git && \\
                cd {repo} && \\
                git checkout -B {branchname} $(git rev-parse --verify {branchname} || echo "") \\
                """,
                shell=True,
            )
            if result.returncode != 0:
                raise Exception("Failed to clone repo or checkout branch")
            progress.update(task, advance=1)


def evaluate_workon(console, ast):
    """Work on a JIRA story: check out the branches and set up the workspace"""
    console.print(f"Working on {ast[1]}")
    proj_slug = slugify(ast[1])
    workdir_path = os.path.join(CHECKOUT_DIR, proj_slug)
    if not os.path.exists(workdir_path):
        console.print(f"Creating {workdir_path}...")
        os.makedirs(workdir_path)
        get_repos(workdir_path)
    command = f"code {' '.join([os.path.join(workdir_path, child) for child in os.listdir(workdir_path)])}"
    console.print(f"Running {command}...")
    subprocess.run(command, shell=True)
